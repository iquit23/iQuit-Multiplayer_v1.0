...
(function (root) {
  ...
  function createHost(cb, desiredCode) {
    ...
    fbReady().then(function (ctx) {
      ...
      // - κενό → δικό μας
      // - δικό μας uid → resume
      // - hostBeat μπαγιάτικο → διαδοχή (host migration) / νεκρό δωμάτιο
      // - αλλιώς → 'unavailable-id' (αν ζητήθηκε συγκεκριμένος κωδικός) ή νέος τυχαίος
      let tries = 0;
      function claim(code) {
        tries++;
        const metaRef = roomRef(ctx.db, code).child('meta');
        metaRef.transaction(function (cur) {
          if (cur === null) return { hostUid: ctx.uid, hostBeat: Date.now(), createdAt: Date.now(), hostSeq: 1 };
          if (cur.hostUid === ctx.uid) { cur.hostBeat = Date.now(); cur.hostSeq = (cur.hostSeq || 1) + 1; return cur; }
          if ((cur.hostBeat || 0) < Date.now() - STALE_TAKEOVER) {
            return { hostUid: ctx.uid, hostBeat: Date.now(), createdAt: cur.createdAt || Date.now(), hostSeq: (cur.hostSeq || 1) + 1 };
          }
          return; // abort — κατειλημμένο από ενεργό host
          ...
          const onMsg = inRef.on('child_added', function (snap) {
            ...
            if (msg.t === 'hello') cb.onHello(v.uid, msg, function (m) { host.sendTo(v.uid, m); });
      ...
      // GC ευκαιρίας: αν ο τυχαίος κωδικός πέσει σε 24ωρο-νεκρό δωμάτιο, το transaction το παίρνει ούτως ή άλλως (stale clause)
